...
static void SetTermGaps(const ProfPos *Prof, unsigned uLength)
	{
	if (0 == uLength)
		return;
...
static SCORE ScoreProfPos(const ProfPos &PPA, const ProfPos &PPB)
	{
	SCORE Score = 0;
...
SCORE GlobalAlign(const ProfPos *PA, unsigned uLengthA, const ProfPos *PB,
  unsigned uLengthB, PWPath &Path)
	...
	MCurr[1] = ScoreProfPos(PA[0], PB[0]);
	SetDPM(1, 1, MCurr[1]);
	SetBitTBM(TB, 1, 1, 'M');
	SetTBM(1, 1, 'M');
	...
	for (unsigned j = 2; j <= uLengthB; ++j)
		{
		MCurr[j] = ScoreProfPos(PA[0], PB[j-1]) + PB[0].m_scoreGapOpen +
...
// Main DP loop
	for (unsigned i = 1; i < uLengthA; ++i)
		...
		if (i == 1)
			...
			{
			MCurr[1] = ScoreProfPos(PA[i-1], PB[0]) + PA[0].m_scoreGapOpen +
		...
		for (unsigned j = 1; j < uLengthB; ++j)
			MNext[j+1] = ScoreProfPos(PA[i], PB[j]);
...
// Special case for i=uLengthA
	...
	MCurr[0] = MINUS_INFINITY;
	if (uLengthA > 1)
		MCurr[1] = ScoreProfPos(PA[uLengthA-1], PB[0]) + (uLengthA - 2)*e +
		  PA[0].m_scoreGapOpen + PA[uLengthA-2].m_scoreGapClose;
	else
		MCurr[1] = ScoreProfPos(PA[uLengthA-1], PB[0]) + PA[0].m_scoreGapOpen +
